...
function genBitmapImage(oData) {
  ...
  //
  //  typedef struct tag BIT_MAP_FILE_HEADER {
  //  WORD bfType;
  //  DWORD bfSize;
  //  WORD bfReserved1;
  //  WORD bfReserved2;
  ...
  //  } BIT_MAP_INFO_HEADER, *PBITMAPINFOHEADER;
  //
  const BIT_MAP_INFO_HEADER = [
  ...
  return encodeData(BIT_MAP_FILE_HEADER.concat(BIT_MAP_INFO_HEADER)) + encodeData(strPixelData);
}
...
function saveAsImage(canvas, width, height, type) {
  ...
  if (/bmp/.test(type)) {
    ...
    saveFile(makeURI(strData, this.downloadMime));
  } else {
    const strData = getDataURL(canvas, type, width, height);
    saveFile(strData.replace(type, this.downloadMime));
  }
}

function convertToImage(canvas, width, height, type, stream) {
  if (typeof canvas == "string") {
    canvas = document.getElementById(canvas);
  }
  if (typeof type === undefined) {
  ...
  if (/bmp/.test(type)) {
    const data = getImageData(scaleCanvas(canvas, width, height));
    const strData = genBitmapImage(data);
    return stream ? strData : genImage(makeURI(strData, 'image/bmp'));
  } else {
    const strData = getDataURL(canvas, type, width, height);
    return stream ? strData : genImage(strData);
  }
}

export class CanvasToImage {
  constructor({ downloadMime } = {}) {
    this.downloadMime = downloadMime || 'image/octet-stream'
  }

  saveAsImage(config) {
    return saveAsImage(config)
  }
  ...
  convertToImage(canvas, width, height, type) {
    return convertToImage(canvas, width, height, type)
  }
  ...
  convertToStream(canvas, width, height, type) {
    return convertToImage(canvas, width, height, type, true)
  }
}
